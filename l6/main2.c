...
int		world_rank;		/* identyfikator w komunikatorze
					 * globalnym */
int		world_size;	/* liczba procesow */

/* Liczy roznice w bajtach pomiedzy wartosciami w indexes */
void 
bytes(int size, int *indexes, int *count, int *skip, int *out_count_bytes, int *out_skip_bytes)
{
	int		i;
	/* 0123456789  */
	/* text	= "oto tekst!" */
...
void 
divide(int size, int nwords, char *words, int *out_count, int *out_skip)
{
	int		i         , j;
	...
	for (i = 0; i < size; i++) {
		...
		if (reminder-- > 0)	/* jesli zostala reszta to dodaj ja
					 * do tego procesu */
			out_count[i]++;

		out_skip[i] = 0;
		/*
		 * dla count = [0, 2, 4, 5] => skip = [0, 0+2, 0+2+4,
		 * 0+2+4+5]
		 */
		for (j = 0; j < i; j++)
...
reduce(int nwords, char *words, int *indexes, int *list)
{
	...
	if (world_rank != master)	/* jesli proces nie jest master, to
					 * trzeba mu przygotowac indexes */
...
/*
 * redukuje mapuje slowa [k1, k2, k1, k3...] z words na [{k1: v1}, {k2: v2},
 * {k1: v3}, {k3: v4} ...]
 */
void 
map(int nwords, char *words, int *indexes, int *out_nwords, char *out_words, int *out_indexes, int *out_occurs)
{
	int		all_words = 0;
	int            *count = NULL;
	int            *count_bytes = NULL;
	...
	fprintf(stderr, "end scatter\n");
	{
		fprintf(stderr, "begin gather\n");
		int		i         , j;
		int		len = 0;
		int            *occurs = NULL;
...
main(int argc, char **argv)
{
	...
	if (world_rank == master) {
		...
		char           *w;
		FILE           *f = fopen("book", "r");
		words = calloc(buffer_size, sizeof(char));	/* "The last time I
								 * was"... */
